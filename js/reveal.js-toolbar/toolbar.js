      ...
      if (initialise) {
        ...
        document.addEventListener('keydown', function(event) {
            if (event.key === 'q' || event.key === 'Q') {
                // 切换显示状态
                dom.toolbar.style.display = dom.toolbar.style.display === 'none' ? 'block' : 'none';
            }
        });
        ...
        if (captureMenu) {
          ...
          var id_menu = setInterval(function() {
            let RevealMenu = Reveal.getPlugin('menu');
