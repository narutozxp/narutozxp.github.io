    ...
    let observer = new MutationObserver(mutations => {
        ...
        for (let i = 0; i < mutations.length; i++) {
            let dom = mutations[i].addedNodes,
                owo_body = '';
            if (dom.length == 2 && dom[1].nodeType === 1 && dom[1].classList.contains('OwO-body')) owo_body = dom[1];
            // 如果需要在评论内容中启用此功能请解除下面的注释
            else if (dom.length == 1 && dom[0].nodeType === 1 && (dom[0].classList.contains('tk-comment') || dom[0].classList.contains('tk-preview-container'))) owo_body = dom[0];
            else continue;
